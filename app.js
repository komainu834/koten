...
/* ==================================================
   Phase7.2〜8：探索マップUI・条件解放・属性アイコン・プレイヤーランク
================================================== */

const ELEMENT_ICON_PATHS = {
  "水": "img/elements/water.png",
  "火": "img/elements/fire.png",
  "風": "img/elements/wind.png",
  "光": "img/elements/light.png",
  "闇": "img/elements/dark.png"
};

const EXPLORE_AREAS = {
  tsukikage: {
    name: "月影の竹林",
    desc: "月光が差し込む静かな竹林。語彙素材を集めやすい。",
    element: "水",
    minutes: 20,
    cost: 10,
    unlockText: "最初から解放",
    requiredRank: 1,
    drops: [
      "img/materials/soul_normal.png",
      "img/materials/crystal_blue.png",
  ...
  yashiro: {
    name: "忘れられた社",
    desc: "古びた社に文法の気配が残る。文法系素材を集めやすい。",
    element: "風",
    minutes: 60,
    cost: 18,
    unlockText: "プレイヤーランク3で解放",
    requiredRank: 3,
    drops: [
      "img/materials/scroll_grammar.png",
      "img/materials/soul_wind.png",
      "img/elements/wind.png"
    ]
  },

  ohmagahara: {
    name: "逢魔ヶ原",
    desc: "妖気が濃い赤き原。助動詞素材を集めやすい。",
    element: "火",
    minutes: 120,
    cost: 30,
    unlockText: "プレイヤーランク5で解放",
    requiredRank: 5,
    drops: [
      "img/materials/crystal_red.png",
      "img/materials/soul_fire.png",
  ...
  kosenjo: {
    name: "幽玄の古戦場",
    desc: "古語の怨念が眠る地。高レア素材を狙える。",
    element: "闇",
    minutes: 180,
    cost: 45,
    unlockText: "プレイヤーランク8で解放",
    requiredRank: 8,
    drops: [
      "img/materials/crystal_purple.png",
      "img/materials/scroll_jodoushi.png",
  ...
  tenko: {
    name: "天狐の禁域",
    desc: "天狐が守る神域。覚醒素材を狙える高難度探索地。",
    element: "光",
    minutes: 360,
    cost: 70,
    unlockText: "プレイヤーランク12で解放",
    requiredRank: 12,
    drops: [
      "img/materials/crystal_rainbow.png",
      "img/materials/soul_light.png",
      "img/elements/light.png"
    ]
  }
};

let selectedExploreArea =
  localStorage.getItem("selectedExploreArea") ||
  "tsukikage";

let playerRank =
  Number(localStorage.getItem("playerRank") || 1);

let playerExp =
  Number(localStorage.getItem("playerExp") || 0);

function getRequiredPlayerExp(rank) {
  return 100 + (rank - 1) * 80;
}

function savePhase8Data() {
  localStorage.setItem("selectedExploreArea", selectedExploreArea);
...
function addPlayerExp(amount) {
  ...
  while (playerExp >= getRequiredPlayerExp(playerRank)) {
    playerExp -= getRequiredPlayerExp(playerRank);
    playerRank += 1;
  }

  savePhase8Data();
  updatePlayerRankUI();
  updateExploreUnlockUI();
}

function updatePlayerRankUI() {
  setText("playerRankText", playerRank);

  const need =
    getRequiredPlayerExp(playerRank);

  setText("playerExpText", playerExp + " / " + need);

  const fill =
    document.getElementById("playerExpFill");

  if (fill) {
    fill.style.width =
      Math.min(100, Math.floor(playerExp / need * 100)) + "%";
  }
}

function isExploreAreaUnlocked(key) {
  const area =
    EXPLORE_AREAS[key];
  ...
  return playerRank >= area.requiredRank;
}

function selectExploreArea(key) {
  selectedExploreArea =
    key;
...
function updateExploreAreaUI() {
  const area =
    EXPLORE_AREAS[selectedExploreArea];

  if (!area) {
    return;
  }

  setText("areaNameText", area.name);
  setText("areaDescText", area.desc);
  setText("areaTimeText", area.minutes + "分");
  setText("areaCostText", area.cost);
  setText("areaElementText", area.element);
  setText("areaUnlockText", area.unlockText);

  const icon =
    document.getElementById("areaElementIcon");

  if (icon) {
    icon.src =
      ELEMENT_ICON_PATHS[area.element];
  }

  const drops =
    document.getElementById("areaDrops");

  if (drops) {
    drops.innerHTML = "";

    area.drops.forEach(function (src) {
      const img =
  ...
  const startButton =
    document.getElementById("areaStartButton");

  if (startButton) {
    if (isExploreAreaUnlocked(selectedExploreArea)) {
      startButton.disabled = false;
      startButton.textContent = "探索開始";
    } else {
      startButton.disabled = true;
      startButton.textContent = "封印中";
    }
  }
...
function updateExploreUnlockUI() {
  Object.keys(EXPLORE_AREAS).forEach(function (key) {
    const node =
      document.getElementById("node_" + key);
    ...
    node.classList.toggle(
      "selected",
      selectedExploreArea === key
    );
  });
}

function startSelectedExplore() {
  const area =
    EXPLORE_AREAS[selectedExploreArea];
...
const oldShowExploreTabPhase8 =
  showExploreTab;

showExploreTab = function () {
  oldShowExploreTabPhase8();

  updatePlayerRankUI();
  updateExploreAreaUI();
};

const oldGiveGameRewardPhase8 =
  giveGameReward;

giveGameReward = function (score, mode) {
  oldGiveGameRewardPhase8(score, mode);
...
const oldUpdateShikigamiDetailPhase8 =
  updateShikigamiDetail;

updateShikigamiDetail = function () {
  oldUpdateShikigamiDetailPhase8();
  ...
  if (master && icon) {
    icon.src =
      ELEMENT_ICON_PATHS[master.element];
  }
};
